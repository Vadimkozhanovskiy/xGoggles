...
public class ConfigHandler
{
	...
	public static void setup(FMLPreInitializationEvent event )
	{
		...
		config.load();
		xGoggles.distIndex = config.get(config.CATEGORY_GENERAL, "searchdist", 0).getInt(); // Get our search distance.
		...
		for( String category : config.getCategoryNames() ) // Iterate through each category in our config file.
		{
			...
			if( category.startsWith( "oredict.") ) // Dont iterate over the base category and make sure were on the oredict category.
			{
				...
				String aspect = cat.get("aspect").getString();

				xGoggles.oredictOres.put(dictName, new OreInfo( guiName, id, meta, color, enabled, aspect) );
			...
			else if( category.startsWith("customores.") )
			{
				...
				xGoggles.customOres.add( new OreInfo( name, id, meta, color, enabled, aspect) );
			}
		}
		config.save();
	}

	public static void add( String oreName, String ore, int color, String aspect)
    {
		config.load();
		...
		for( String category : config.getCategoryNames() )
		{
			if( category.startsWith("customores.") )
			{
				...
				{
					String notify = String.format( "[xGoggles] %s already exists. Please enter a different name. ", oreName );
		...
		for( String category : config.getCategoryNames() )
		{
			if( category.startsWith("customores.") )
			{
				...
				config.get("customores."+formattedname, "aspect", "").set(aspect);
			}
		}
		config.save();
	}
	...
	public static void update(String string, boolean draw){
		...
		{
			config.get(config.CATEGORY_GENERAL, "searchdist", 0).set( xGoggles.distIndex );
			config.save();
			return;
		}
